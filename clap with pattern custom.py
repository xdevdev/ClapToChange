    ...
    if amplitude > tap_threshold: # if its to loud...
        ...
        while True:
            ...
            if 1 <= noisycount <= MAX_TAP_BLOCKS:
                ...
                if current - start <= 3:
                    loop_count_list.append(current)

                else:

                    if clap_count == 3:

                        if loop_count_list[2] - loop_count_list[1] <= 0.8:
                            print('pattern 1 is successful!')

                            clap_count = 0
                            loop_count_list = []

                        elif loop_count_list[1] - loop_count_list[1] <= 0.8:
                            print('pattern 2 is successful!')

                            clap_count = 0
                            loop_count_list = []

                        else:
                            print('No pattern successful!')
